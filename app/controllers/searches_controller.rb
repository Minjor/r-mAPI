class SearchesController < ApplicationController
    def show
        ...
        @episodes = JSON.parse(HTTP.get('https://rickandmortyapi.com/api/episode/?name=' + @word))
        if @episodes['results']
            @episode_list = Array.new
            @episodes['results'].each { |episode| @episode_list.push episode }
        end
        
        @characters = JSON.parse(HTTP.get('https://rickandmortyapi.com/api/character/?name=' + @word))
        if @characters['results']
            @character_list = Array.new
            @characters['results'].each { |character| @character_list.push character }
        end

        @locations = JSON.parse(HTTP.get('https://rickandmortyapi.com/api/location/?name=' + @word))
